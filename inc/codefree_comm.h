...
/**
 * @defgroup codefree_comm_types Type Definitions Data types for the 80339 driver.
 * @{
 */
 ...
 * - RX complete -> callback (checksum is validated and filled into crcCorrect field)
 * - Timeout -> chip did not respond to read request in time.
 */
typedef void (*CodefreeComm_ReadCallback_t)(CodefreeCommReadMessage_t* msg, CodefreeCommEvent_t event);
...
/**
 * @defgroup codefree_comm_functions Function Definitions Function prototypes for the 80339 driver.
 * @{
 */
 ...
 *
 * @param appCallback pointer to function that is to be executed when receiving data.
 ...
 */
void codefree_comm_SetActiveCommand(uint8_t slvAddr, CodefreeCommSupportedCommands_t newCommandType);
 ...
 * This function copies the received data in to the application-provided read buffer.
 * It also executes the checkCrc, and executes the application-provided function.
 */
