 ...
 * This module can be used to send commands to the codefree 80339 product via
 * I2C.
 */
...
#pragma error("define a CODEFREE_COMM_I2C_DRIVER_INSTANCE for the mlx codefree communication block.")
#endif
...
bool codefree_comm_ext_sendWriteMessage(CodefreeCommWriteMessage_t *msg) {
  ...
  if (i2cDrv->GetStatus().busy) {
    ...
    if (i2cDrv->MasterTransmit(msg->slaveAddress, messageBuffer, 7, false) !=
        ARM_DRIVER_OK) {
      retVal = false;
    }
  }
...
bool codefree_comm_ext_sendReadRequest(
  ...
  /* Non-blocking check: Do not execute if hardware is still
   * transmitting/receiving */
  if (i2cDrv->GetStatus().busy) {
    retVal = false;
  } else {
    if (i2cDrv->MasterReceive(readMsgStruct->slaveAddress,
                              readMsgStruct->readResponse, 8,
                              false) != ARM_DRIVER_OK) {
      retVal = false;
    }
  }
...
static inline void codefree_comm_ext_callback(uint32_t event) {
  ...
  /* 1. Transfer successfully finished (both TX and RX complete trigger this in
   * I2C) */
  if (event & ARM_I2C_EVENT_TRANSFER_DONE) {
    if (codefree_comm_getStatus() == CODEFREE_COMM_STATUS_SENDING_READ) {
      codefree_comm_handleRxComplete();
      /* Note: Ensure handleRxComplete updates the status to IDLE or appropriate
       * next state */
  ...
  uint32_t errorMask = ARM_I2C_EVENT_TRANSFER_INCOMPLETE |
                       ...
                       ARM_I2C_EVENT_ARBITRATION_LOST | ARM_I2C_EVENT_BUS_ERROR;
